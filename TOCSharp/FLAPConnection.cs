...
public class FLAPConnection
{
    ...
    private ushort seqNo = (ushort)Random.Shared.Next(ushort.MaxValue);
    ...
    public async Task ConnectAsync()
    {
        ...
        await this.socket.SendAsync("FLAPON\r\n\r\n"u8.ToArray());
        this.Connected = true;
    ...
    private async Task FLAPActivity()
    {
        while (this.Connected)
        {
            try
            {
                ...
                this.buffer = this.buffer.Length != 0 ? [..this.buffer, ..bytes] : bytes;
    ...
    public async Task SendPacketAsync(FLAPPacket packet)
    {
        ...
        await this.socket.SendAsync(packet.ToBytes());
    }
}